...
var static = require("serve-static"); //정적자원 처리를 위한 외부모듈 (외부설치)
var ejs = require("ejs"); //외부설치
var mysql = require("mysql"); //외부설치
var path = require("path") //확장자 추출 등 파일 경로와 관련된 처리 모듈
...
app.get("/admin/main",function(request, response){
    //main.ejs는 아무나 보여주면 안된다 즉 인증을 거친사람만이 볼수있는 페이지다
...
app.get("/admin/product/list", function(request, response){
    ...
    //1페이지가 아닌경우는 누군가가 페이지 아래 링크를 눌렀다면 currentPage 파라미터가 넘어온다
    ...
    con.query(sql, function(err, result, fields){
        if(err){
            console.log("상품 리스트 가져오기 실패",err);
        }else{
            console.log(result);
