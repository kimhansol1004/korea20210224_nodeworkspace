...
var qs=require("querystring"); //포스트형식으로 바디에 들어올때 별도로 필요하다
var url=require("url"); //url분석과 관련된 내부모듈
var mysql=require("mysql"); //mysql 모듈 가져오기(외부 모듈이므로 별도 설치 필요)
...
var server = http.createServer(function(request, response){
    //결국 서버는 클라이언트의 다양한 요청을 처리하기 위해, 요청을 구분할 수 잇어야 한다
    //결룰 클라이언트가 서버에게 무엇을 원하는지에 대한 정보는 요청 URL 로 구분할 수 있다
...
function registForm(request, response){
    fs.readFile("./regist_from.html","utf8",function(err,data){
...
function regist(request, response){
    //클라이언트가 post 방식으로 전송했기 때문에 http 데이터 구성 중 body를 통해 전송되어 온다
    ...
    request.on("data",function(param){ 
        //param에는 body안에 들어있는 데이터가 서버의 메모리 버퍼로 들어 오고, 
        //그 데이터를 param이 담고 있다
        content+=param; //버퍼의 데이터를 모으자
    }); //post방식의 데이터를 감지
    
    //데이터가 모두 전송되어, 받아지면 and 이벤트 발생
    request.on("end",function(){
        ...
        //이 시점이 쿼리문을 수행할 시점임
        //데이터베이스에 쿼리문을 전송하기 위해서는, 먼저 접속이 선행되어야 한다 (당연하)
       ...
       con.query(sql, function(err, fields){
            if(err){ //쿼리수행중 심각한 에러 발생
                ...
                response.writeHead(200, {"Content-Type":"text/html; charset=utf-8"});
                esponse.end("회원가입 성공<br><a href='/member/list'>회원목록 바로가기</a>");
...
function getList2(request, response){
    ...
    con.query(sql,function(err, record, fields){
        //record 변수에는 json들이 일차원 배열에 탑재되어 있다
...
//유지보수성이 너무 낮다 따라서 프로그램 코드와 디자인은 분리 되어야 좋다
...
function getDetail(request, response){
    ...
    //추출하기가 용이하다 (마치 봉투에 겉면에 씌여진 글씨와 같다)
    
    //개발자가 직접 url을 문자열 분석을 시도하기 보다는 보다 url을 전문적으로 해석 및 분석할 수 있는 
    //모듈에게 맡기면 된다 그 역할을 수행하는 모듈이 바로  url 내부모듈 이다!
...
function del(request, response){
    ...
    //커낵션 객체는 접속 정보를 가진 객체이며, 이 객체를 통해 쿼리를 수행할 수 있다
    //또한 mysql 접속을 해제도 가능하다
    //select문의 경우엔 결과를 가져와야 하기 대문에 가운데 인수, result인수가 필요하지만
