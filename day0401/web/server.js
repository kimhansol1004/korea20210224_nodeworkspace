...
var fs = require("fs"); //fs 모듈 가져오기 - 파일을 다룰대 사용되는 모듈
...
var server = http.createServer(function(request, response){
    ...
    //그 조사 결과에 따라서 아래 코드중 어떤 코드가 실행될지를 결정짓자
    ...
    if(url=="/member/form"){
        //회원가입을 희망하는 사람이라면
        fs.readFile("./regist_from.html" ,"utf8", function(error, data){
            //파일의 내용을 모두 읽어들인 순간 이 익명함수가 동작한다
            response.end(data);
        });
    }else if(url=="/member/regist"){ //등록을 원하면..
        //쿼리문 수행 전에, node.js가 mysql에 접속을 성공해야 한다
        var con=mysql.createConnection({
            url:"localhost:3306", /*db는 네트워트 프로그램이라 포트 사용함 */
    ...
    }else if("/member/result"){ //회원등록을 성공한 사람이 보게될 화면
